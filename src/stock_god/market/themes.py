...
def normalize(text):
    return "".join(
        ...
        if not char.isspace() and unicodedata.category(char)[0] not in "PS"
    )
...
def normalize_ref(raw):
    ...
    query = [
        ...
        if not key.lower().startswith("utm_") and key.lower() not in {"spm", "from", "share_token"}
    ]
...
class Themes(ProviderState):
    ...
    def _theme_snapshot(self, theme_id, day="", cutoff=None):
        ...
        rows = database_rows(
            self.config.main_db,
            "SELECT * FROM market_theme_daily_snapshots WHERE "
            + condition
    ...
    def _theme_claims(self, event_id, cutoff) -> list[dict]:
        rows = database_rows(
            self.config.main_db,
        ...
        for row in rows:
            links = database_rows(
                self.config.main_db,
                "SELECT evidence_item_id FROM market_theme_evidence_links WHERE source_claim_id=? ORDER BY evidence_item_id",
                (row["source_claim_id"],),
            )
            result.append(
                camel(row) | {"evidenceItemIds": sorted({item["evidence_item_id"] for item in links})}
            )
        return result
    ...
    def _theme_catalysts(self, snapshot):
        ...
        for row in rows:
            ...
            result.append(
                camel(row) | {"sources": claims, "hasConflict": {"supports", "contradicts"} <= stances}
            )
        return result
    ...
    def theme_api(self, theme_id, mode, query):
        ...
        try:
            offset = (
                int(base64.urlsafe_b64decode(cursor + "=" * ((4 - len(cursor) % 4) % 4)).decode())
                if cursor
                else 0
            )
            if offset < 0:
        ...
        if mode == "list":
            ...
            if not day:
                dates = database_rows(
                    self.config.main_db,
                    "SELECT MAX(trade_date) AS trade_date FROM market_theme_daily_snapshots",
                )
                day = dates[0]["trade_date"] or now().date().isoformat()
            snapshots = database_rows(
                self.config.main_db,
                "SELECT s.*,t.canonical_name FROM market_theme_daily_snapshots s JOIN market_themes t ON t.theme_id=s.theme_id WHERE trade_date=?",
                (day,),
            )
            items = []
            for row in snapshots:
                if (
                    stage
                    and row["lifecycle_stage"] != stage
                    or query.get("q", "").lower() not in row["canonical_name"].lower()
                ):
                    continue
                aliases = [
                    value["alias"]
                    for value in database_rows(
                        self.config.main_db,
                        "SELECT alias FROM market_theme_aliases WHERE theme_id=?",
                        (row["theme_id"],),
                    )
                ]
                previous = database_rows(
                    self.config.main_db,
                    "SELECT lifecycle_stage FROM market_theme_daily_snapshots WHERE theme_id=? AND trade_date<? ORDER BY trade_date DESC LIMIT 1",
                    (row["theme_id"], day),
                )
                constituents = database_rows(
                    self.config.main_db,
                    "SELECT asset_type,market,code,name,role FROM market_theme_snapshot_constituents WHERE snapshot_id=? ORDER BY rank LIMIT 3",
                    (row["snapshot_id"],),
                )
                items.append(
                    camel(row)
                    | {
                        ...
                        "aliases": aliases,
                        "stageChanged": bool(
                            previous and previous[0]["lifecycle_stage"] != row["lifecycle_stage"]
                        ),
                        **({"previousLifecycleStage": previous[0]["lifecycle_stage"]} if previous else {}),
                        "representativeSecurities": [camel(item) for item in constituents],
                    }
                )
            items.sort(
                key=lambda item: (
                    (item["rank"], item["themeId"])
                    if sort == "rank"
                    else (-item["heatScore"], item["themeId"])
                    if sort == "heat"
                    else (STAGES.index(item["lifecycleStage"]), item["rank"])
                )
            )
            ...
            if mode == "snapshots":
                ...
                rows = database_rows(
                    self.config.main_db,
                    "SELECT * FROM market_theme_daily_snapshots WHERE theme_id=? ORDER BY trade_date DESC",
                    (theme_id,),
                )
                items = [
                    camel(row)
                    ...
                    and (not stage or row["lifecycle_stage"] == stage)
                ]
                data = {"themeId": theme_id, **paged(items)}
            elif mode == "catalysts":
                status, minimum = query.get("status", ""), int(query.get("minCredibility") or 0)
                if (
                    status not in {"", "active", "disputed", "retracted", "expired"}
                    or not 0 <= minimum <= 100
                ):
                    raise ValueError("invalid catalyst filter")
                items = [
                    row
                    for row in self._theme_catalysts(snapshot)
                    if (not status or row["status"] == status) and row["credibilityScore"] >= minimum
                ]
                data = {
                    "themeId": theme_id,
                    "tradeDate": snapshot["trade_date"]
                    if snapshot
                    else query.get("date", now().date().isoformat()),
                    **paged(items),
                }
            else:
                aliases = [
                ...
                constituents = (
                    database_rows(
                        self.config.main_db,
                        "SELECT * FROM market_theme_snapshot_constituents WHERE snapshot_id=? ORDER BY rank",
                ...
                data = {
                    ...
                    "snapshot": camel(snapshot) if snapshot else None,
                    "constituents": [camel(row) for row in constituents],
                    "catalystSummary": {
                        "total": len(catalysts),
                        "supports": sum(row["stance"] == "supports" for row in claims),
                        "contradicts": sum(row["stance"] == "contradicts" for row in claims),
                        "hasConflict": any(row["hasConflict"] for row in catalysts),
                    },
                }
        result = envelope(
            data,
            "theme_repository",
            as_of=as_of,
            status="empty" if mode != "detail" and not data.get("items") else "ok",
        )
    ...
    def prediction_theme_documents(self, cutoff):
        ...
        for row in rows:
            ...
            documents.append(
                {
                    "sourceId": "theme:" + row["snapshot_id"],
                    "sourceName": data["data"]["theme"]["name"],
                    "sourceRef": "db:market_theme_daily_snapshots/" + row["snapshot_id"],
                    "category": "theme",
                    "collectedAt": now().isoformat(),
                    "retrievedAt": now().isoformat(),
                    "availableAt": row["frozen_at"],
                    "publishedAt": row["frozen_at"],
    ...
    def collect_theme_signals(self, at):
        sources = [
            ("hot_topic", "东方财富热门话题", lambda: self.hot_topics(30)),
            ("hot_event", "雪球热点事件", lambda: self.hot_events(30)),
            ("news", "新闻/电报", lambda: self.telegraphs(limit=100)),
            ("fund_flow", "概念资金流", lambda: self.fund_flows("concept", limit=50)),
        ]
            ...
            try:
                ...
                for rank, row in enumerate(payload, 1):
                    name = next(
                        (
                            str(row[key]).strip()
                            for key in ("TopicName", "topicName", "tag", "name", "title")
                            if row.get(key)
                        ),
                        "",
                    )
                    ...
                    if not strength:
                        ...
                        if kind == "fund_flow":
                            strength += (number(row.get("changePct"), 0) * 3) + (
                                5
                                if number(row.get("netAmount"), 0) > 0
                                else -5
                                if number(row.get("netAmount"), 0) < 0
                                else 0
                            )
                    ...
                    securities = []
                    for value in row.get("stocks", []):
                        try:
                            securities.append(instrument(value) | {"name": "", "role": ""})
                        except ValueError:
                            continue
                    observed = timestamp(row["CreatedAt"]) if row.get("CreatedAt") else timestamp(at)
                    for theme_name in row.get("subjects") or [name]:
                        result.append(
                            {
                                "name": theme_name,
                                "title": title,
                                "summary": row.get("summary")
                                or row.get("TopicDesc")
                                or row.get("content")
                                or title,
                                "source": source_name,
                                "kind": kind,
                                ...
                                "published": event_at.isoformat() if published else None,
                                "available": max(observed, event_at).isoformat()
                                if published
                                else observed.isoformat(),
                                "ref": normalize_ref(
                                    row.get("url")
                                    or "urn:stock-god:theme-source:"
                                    + digest(json.dumps(row, ensure_ascii=False, sort_keys=True))
                                ),
                                "stance": "contradicts"
                                if row.get("sentimentResult") in ("negative", "利空", "负面")
                                else "supports",
                                "rawHash": digest(json.dumps(row, ensure_ascii=False, sort_keys=True)),
                                "securities": securities,
                            }
                        )
                return (
                    result,
                    {"source": source, "status": "ok" if result else "empty", "signalCount": len(result)},
                    None,
                )
            except (MarketDataError, ValueError, KeyError) as exc:
                return (
                    [],
                    {"source": source, "status": "unavailable", "signalCount": 0},
                    {"source": source, "code": "unavailable", "message": str(exc)},
                )

        with ThreadPoolExecutor(max_workers=4) as pool:
            results = list(pool.map(collect, sources))
        unique = {}
        for values, _, _ in results:
            for signal in values:
                key = "|".join(
                    str(signal[field]).strip().lower()
                    for field in ("name", "kind", "title", "summary", "source", "ref", "stance", "at")
                )
                unique[key] = signal
        return (
            list(unique.values()),
            [state for _, state, _ in results],
    ...
    def refresh_themes(self, at):
        ...
        frozen = max([now(), at] + [timestamp(signal["available"]) for signal in signals]).isoformat()
        result = {
            "status": "unavailable",
            "tradeDate": at.date().isoformat(),
        ...
        groups = {}
        aliases = {
            row["normalized_alias"]: row["normalized_name"]
            for row in database_rows(
                self.config.main_db,
                "SELECT a.normalized_alias,t.normalized_name FROM market_theme_aliases a JOIN market_themes t ON t.theme_id=a.theme_id",
            )
        }
        for signal in signals:
            key = normalize(signal["name"])
            if key:
                groups.setdefault(aliases.get(key, key), []).append(signal)
        ranked = []
        for key, group in groups.items():
            heat = min(
                100,
                max(row["strength"] for row in group)
                + min(15, (len({row["source"] for row in group}) - 1) * 5)
                + min(10, (len(group) - 1) * 2),
            )
            ...
            try:
                with sqlite3.connect(self.config.main_db, timeout=10) as connection:
                    ...
                    identity = connection.execute(
                        "SELECT * FROM market_themes WHERE normalized_name=? OR theme_id IN (SELECT theme_id FROM market_theme_aliases WHERE normalized_alias=?) LIMIT 1",
                        (key, key),
                    ).fetchone()
                    ...
                    existing = connection.execute(
                        "SELECT * FROM market_theme_daily_snapshots WHERE theme_id=? AND trade_date=?",
                        (theme_id, at.date().isoformat()),
                    ).fetchone()
                    if existing:
                        snapshot = dict(existing)
                    else:
                        previous = connection.execute(
                            "SELECT * FROM market_theme_daily_snapshots WHERE theme_id=? AND trade_date<? ORDER BY trade_date DESC LIMIT 1",
                            (theme_id, at.date().isoformat()),
                        ).fetchone()
                        catalyst_ids, conflicting = [], set()
                        for signal in group:
                            ...
                            entities = sorted(
                                {
                                    normalize(item["market"] + item["code"])
                                    for item in signal.get("securities", [])
                                }
                            )
                            event_key = digest(
                                "|".join(
                            ...
                            event = connection.execute(
                                "SELECT * FROM market_catalyst_events WHERE catalyst_event_id=?", (event_id,)
                            ).fetchone()
                            if not event:
                                insert(
                                    connection,
                                    "market_catalyst_events",
                                    {
                                        ...
                                        "theme_id": theme_id,
                                        "event_fingerprint": digest(normalize(theme_id) + "|" + event_key),
                                        "event_type": signal["kind"],
                                        "title": signal["title"],
                                        "summary": signal["summary"],
                                        "event_at": signal["at"],
                                        "first_available_at": signal["available"],
                                        "credibility_score": 65
                                        if signal["kind"] == "hot_topic"
                                        else 60
                                        if signal["kind"] == "hot_event"
                                        else 70,
                                        "status": "active",
                                        "entity_keys_json": json.dumps(entities),
                                        "created_at": frozen,
                                    },
                                )
                            ref_hash = digest(normalize_ref(signal["ref"]))
                            claim = connection.execute(
                                "SELECT 1 FROM market_catalyst_source_claims WHERE catalyst_event_id=? AND source_ref_hash=?",
                                (event_id, ref_hash),
                            ).fetchone()
                            if not claim:
                                insert(
                                    ...
                                    "market_catalyst_source_claims",
                                    {
                                        ...
                                        "source_ref": signal["ref"],
                                        "source_ref_hash": ref_hash,
                                        "stance": signal["stance"],
                                        "source_credibility_score": 65,
                                        "summary": signal["summary"],
                                        "claim_fingerprint": digest(normalize(signal["summary"])),
                                        "published_at": signal["published"],
                                        "available_at": signal["available"],
                                        "collected_at": frozen,
                                        "raw_payload_hash": signal["rawHash"],
                                        "created_at": frozen,
                                    },
                                )
                            stances = {
                                ...
                                for row in connection.execute(
                                    "SELECT stance FROM market_catalyst_source_claims WHERE catalyst_event_id=?",
                                    (event_id,),
                                )
                            }
                            ...
                            if disputed:
                                connection.execute(
                                    "UPDATE market_catalyst_events SET status='disputed' WHERE catalyst_event_id=?",
                                    (event_id,),
                                )
                            if disputed:
                        ...
                        cycle, stage = next_stage(
                            dict(previous) if previous else None, heat, rank, count, bool(conflicting)
                        )
                        securities = {}
                        for signal in group:
                            for item in signal.get("securities", []):
                                security_key = item["assetType"] + "|" + item["market"] + "|" + item["code"]
                                stored = securities.get(security_key)
                                if stored is None or stored["contributionScore"] < signal["strength"]:
                                    securities[security_key] = item | {
                                        "contributionScore": signal["strength"]
                                    }
                        ordered = sorted(
                            securities.values(), key=lambda item: (-item["contributionScore"], item["code"])
                        )
                        summary = "；".join(
                            dict.fromkeys(
                                row["summary"] for row in sorted(group, key=lambda row: -row["strength"])
                            )
                        )[:1200]
                        snapshot = {
                            ...
                            "content_hash": digest(
                                json.dumps(
                                    [theme_id, at.isoformat(), cycle, stage, heat, summary, catalyst_ids],
                                    ensure_ascii=False,
                                )
                            ),
                            "constituent_count": len(ordered),
                            "catalyst_count": len(catalyst_ids),
                            "conflicting_catalyst_count": len(conflicting),
                            "created_at": frozen,
                        }
