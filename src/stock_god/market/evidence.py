...
FLOW_SORT = {
    "netamount": "f62",
    "main_ratio": "f184",
    "super_large_netamount": "f66",
    "large_netamount": "f72",
    "medium_netamount": "f78",
    "small_netamount": "f84",
    "change_pct": "f3",
    "avg_changeratio": "f3",
    "inamount": "f62",
...
def breadth_data(rows):
    ...
    for row in observations:
        ...
        upper = float(
            (Decimal(str(previous)) * (1 + Decimal(str(fraction)))).quantize(
        ...
        up += row["price"] >= upper - 0.00001
        down += row["price"] <= lower + 0.00001
    return {
...
class Evidence(ProviderState):
    def breadth(self):
        def load():
            try:
                ...
                return envelope(
                    breadth_data(snapshot["rows"]),
                    snapshot["source"],
                    as_of=at,
                    status="partial" if snapshot["errors"] else "ok",
    ...
    def fund_flows(self, scope, day="", sort="netamount", limit=20):
        ...
        def primary():
            payload = self.http.json(
                "https://push2delay.eastmoney.com/api/qt/clist/get",
                {
                    ...
                    "invt": 2,
                    "fid": FLOW_SORT[sort],
                    "fs": "m:90 s:4" if scope == "sector" else "m:90 t:3",
                    "stat": 1,
                ...
                rows.append(
                    {
                        "code": value["f12"],
                        "name": value.get("f14", ""),
                        "netAmount": number(value["f62"]),
                        "inAmount": None,
                        "outAmount": None,
                        **{
                            key: number(value.get(field))
                            for key, field in {
                                "changePct": "f3",
                                "mainNetRatio": "f184",
                                "superLargeNetAmount": "f66",
                                "largeNetAmount": "f72",
                                "mediumNetAmount": "f78",
                                "smallNetAmount": "f84",
                            }.items()
        ...
        def fallback():
            ...
            values = array(
                self.http.json(
                    SINA + "ssl_bkzj_bk",
                    {
                        ...
                        "sort": sort,
                        "asc": 0,
                        "fenlei": "0" if scope == "sector" else "1",
                    },
                )
            )
            rows = []
            for value in values:
                ...
                rows.append(
                    {
                        "code": str(value.get("category", value.get("code", ""))),
                        "name": value.get("name", value.get("categoryname", "")),
                        "netAmount": net,
                        "inAmount": number(value.get("inamount")),
                        "outAmount": number(value.get("outamount")),
                        "changePct": change * 100 if change is not None else None,
                        "mainNetRatio": None,
                        "superLargeNetAmount": None,
                        "largeNetAmount": None,
                        "mediumNetAmount": None,
                        "smallNetAmount": None,
                    }
                )
            if not rows:
    ...
    def fund_flow_timeline(self, code):
        ...
        def load():
            ...
            for line in data["klines"]:
                ...
                points.append(
                    {
                        "at": timestamp(fields[0]).isoformat(),
                        **{
    ...
    def futures_positions(self, symbol, day=""):
        metadata = {
            "IF": ("沪深300股指期货", "sh000300"),
            "IH": ("上证50股指期货", "sh000016"),
            "IC": ("中证500股指期货", "sh000905"),
            "IM": ("中证1000股指期货", "sh000852"),
        }
        ...
        def primary():
            contracts = array(
                self.http.json(
                    endpoint,
                    {
                        ...
                        "pageSize": 5,
                        "pageNumber": 1,
                        "filter": f'(TRADE_CODE="{symbol}")(IS_MAINCODE="1")',
                    },
                ),
                "result",
                "data",
            ...
            rows = [
                {
                    "tradeDate": value["TRADE_DATE"][:10],
                    **{
                        key: number(value.get(field), 0)
                        for key, field in {
                            "settlePrice": "SETTLE_PRICE",
                            "longPosition": "TOTAL_LONG_POSITION",
                            "longChange": "LP_CHANGE_TOTAL",
                            "shortPosition": "TOTAL_SHORT_POSITION",
                            "shortChange": "SP_CHANGE_TOTAL",
                            "netPosition": "NET_POSITION",
                            "indexClose": "CLOSE_PRICE",
                            "indexChange": "CLOSE_PRICE_CHANGE",
                            "basis": "BASIS",
                        }.items()
                    },
                }
                for value in values
                if not day or value["TRADE_DATE"].startswith(day)
            ]
            if not rows:
        ...
        def fallback():
            ...
            for offset in range(1 if day else 7):
                ...
                try:
                    text = self.http.text(
                        f"http://www.cffex.com.cn/sj/ccpm/{date:%Y%m}/{date:%d}/{symbol}_1.csv",
                        encoding="gb18030",
                    )
                ...
                members = [
                    row
                    for row in list(csv.reader(io.StringIO(text)))[2:]
                    if len(row) >= 12 and row[2].strip().isdigit()
                ]
                totals = {}
                for row in members:
                ...
                if not totals:
                    continue
                contract = max(totals, key=lambda key: totals[key])
                    ...
                    "rows": [
                        {
                            ...
                            "longPosition": longs,
                            "shortPosition": shorts,
                            "netPosition": longs - shorts,
                            "settlePrice": 0,
                            "longChange": 0,
                            "shortChange": 0,
                            "indexClose": 0,
    ...
    def margin(self, scope="market", code="", day=""):
        ...
        if scope == "security":
            ...
            def security():
                values = array(
                    self.http.json(
                        "https://datacenter.eastmoney.com/securities/api/data/v1/get",
                        {
                            ...
                            "pageNumber": 1,
                            "pageSize": 50,
                            "filter": f'(SECUCODE="{code[2:]}.{code[:2].upper()}")',
                        },
                    ),
                    "result",
                    "data",
                ...
                rows = [
                    {
                        "date": value["TRADE_DATE"][:10],
                        "code": value.get("SECURITY_CODE", code[2:]),
                        "name": value.get("SECURITY_NAME_ABBR", ""),
                        **{
                            key: number(value.get(field), 0)
                            for key, field in {
                                "financingBalance": "FIN_BALANCE",
                                "securitiesBalance": "LOAN_BALANCE",
                                "marginBalance": "MARGIN_BALANCE",
                                "financingBuy": "FIN_BUY_AMT",
                                "securitiesSell": "LOAN_SELL_VOL",
                            }.items()
                        },
                    }
                    for value in values
        ...
        for exchange in ("SSE", "SZSE"):
            try:
                if exchange == "SSE":
                    ...
                    mapping = {
                        "financingBalance": "rzye",
                        "securitiesBalance": "rqylje",
                        "marginBalance": "rzrqjyzl",
                        "financingBuy": "rzmre",
                        "securitiesSell": "rqmcl",
                    }
                    scale = 1
                    ...
                    found = re.search(
                        r"\d{4}-\d{2}-\d{2}", str(payload[0].get("metadata", {}).get("subname", ""))
                    )
                    date = found.group() if found else target_day
                    mapping = {
                        "financingBalance": "jrrzye",
                        "securitiesBalance": "jrrjye",
                        "marginBalance": "jrrzrjye",
                        "financingBuy": "jrrzmr",
                        "securitiesSell": "jrrjmc",
                    }
                    scale = 1e8
                ...
                rows.append(
                    {
                        "code": exchange,
                        "name": "上海证券交易所" if exchange == "SSE" else "深圳证券交易所",
                        "date": date,
        ...
        return envelope(
            ...
            "sse+szse",
            status="partial" if rows and errors else "ok" if rows else "unavailable",
            as_of=target_day,
            errors=errors,
        )

    def trades(self, code, asset_type="stock", day="", cursor=0, limit=100, *, include_previous_close=False):
        code = evidence_instrument(code, asset_type)["code"]
        ...
        def cached():
            ...
            items = [
                {
                    "time": timestamp(value["traded_at"]).strftime("%H:%M:%S"),
                    **{key: value[key] for key in ("price", "volume", "amount", "side")},
                }
                for value in values[:limit]
            ]
            return empty | {
                "items": items,
                **({"nextCursor": str(cursor + limit)} if len(values) > limit else {}),
            }, timestamp(values[min(len(values), limit) - 1]["traded_at"])
        ...
        def live():
            ...
            for line in array(payload, "data", "details"):
                ...
                if len(fields) < 3 or number(fields[1]) is None or number(fields[2]) is None:
                    continue
                price, volume = float(fields[1]), float(fields[2])
                items.append(
                    {
                        "time": fields[0] if len(fields[0]) == 8 else fields[0] + ":00",
                        "price": price,
                        "volume": volume,
                        ...
                        "side": {
                            ...
                            "4": "neutral",
                            "N": "neutral",
                        }.get(fields[4] if len(fields) > 4 else "", ""),
                    }
                )
            if not items:
            ...
            try:
                if not self.config.minute_db.is_file():
                    raise sqlite3.OperationalError("minute database unavailable")
                with sqlite3.connect(self.config.minute_db, timeout=10) as connection:
                    for sequence, item in enumerate(items):
                        at = timestamp(day + "T" + item["time"])
                        connection.execute(
                            "INSERT INTO market_trade_tick(asset_type,symbol,traded_at,sequence,price,volume,amount,side,source,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?) ON CONFLICT(asset_type,symbol,traded_at,sequence) DO UPDATE SET price=excluded.price,volume=excluded.volume,amount=excluded.amount,side=excluded.side,updated_at=excluded.updated_at",
                            (
                                asset_type,
                                code,
                ...
                cache_errors.append({"provider": "cache", "code": "cache_write_failed", "message": str(exc)})
            data = empty | {
                "items": items[cursor : cursor + limit],
                **({"nextCursor": str(cursor + limit)} if len(items) > cursor + limit else {}),
            }
            if include_previous_close:
                data["_previousClose"] = number(payload.get("data", {}).get("prePrice"))
            return data, day + "T" + items[-1]["time"]

        result = self.http.chain([("eastmoney", live), ("local-trade-cache", cached)], empty)
        if cache_errors and result["status"] != "unavailable":
            result["status"] = "partial"
            result["errors"].extend(cache_errors)
        return result
    ...
    def auction(self, code, asset_type="stock", day=""):
        ...
        def cached():
            rows = database_rows(
                self.config.minute_db,
                "SELECT * FROM market_auction_snapshot WHERE asset_type=? AND symbol=? AND trade_date=? ORDER BY observed_at",
                (asset_type, code, day),
            )
            if not rows:
            ...
            items = [
                {
                    "time": timestamp(row["observed_at"]).strftime("%H:%M:%S"),
                    "price": row["indicative_price"],
                    "matchedVolume": row["matched_volume"],
                    "matchedAmount": row["matched_amount"],
                    "unmatchedVolume": row["unmatched_volume"],
                    "unmatchedSide": row["unmatched_side"],
                }
                for row in rows
                if row["phase"] != "final"
            ]
            final = (
                ...
                if items
                    ...
                    "time": timestamp(rows[-1]["observed_at"]).strftime("%H:%M:%S"),
                    "price": rows[-1]["indicative_price"],
                    "matchedVolume": rows[-1]["matched_volume"],
                    "matchedAmount": rows[-1]["matched_amount"],
                    "unmatchedVolume": rows[-1]["unmatched_volume"],
                }
            )
            strength = (final["price"] / items[0]["price"] - 1) * 100 if items and items[0]["price"] else None
            return empty | {
                "snapshots": items,
                "finalSnapshot": final,
                "auctionStrength": strength,
            }, timestamp(rows[-1]["observed_at"])

        if day != now().date().isoformat():
            return self.http.chain([("local-auction-cache", cached)], empty)
        details = self.trades(code, asset_type, day, 0, 2000, include_previous_close=True)
        selected = [row for row in details["data"]["items"] if "09:15:00" <= row["time"] < "09:30:00"]
        if selected:
            items = [
                {
                    "time": row["time"],
                    "price": row["price"],
                    "matchedVolume": row["volume"],
                    "matchedAmount": row["amount"],
                    "unmatchedVolume": None,
                }
                for row in selected
            ]
            previous = details["data"].get("_previousClose")
            strength = (items[-1]["price"] / items[0]["price"] - 1) * 100 if items[0]["price"] else None
            gap = (items[-1]["price"] / previous - 1) * 100 if previous else None
            result = envelope(
                empty
                | {
                    "snapshots": items,
                    "finalSnapshot": items[-1],
                    "auctionStrength": strength,
                    "gapPct": gap,
                },
                "eastmoney",
                status="partial",
                as_of=day + "T" + items[-1]["time"],
                warnings=["Public trade ticks do not provide unmatched auction volume"],
            )
            try:
                with sqlite3.connect(self.config.minute_db, timeout=10) as connection:
                    for index, item in enumerate(items):
                        observed = timestamp(day + "T" + item["time"])
                        phase = "final" if index == len(items) - 1 else "matched"
                        for kind in (phase,) if phase != "final" else ("matched", "final"):
                            connection.execute(
                                "INSERT INTO market_auction_snapshot(asset_type,symbol,trade_date,observed_at,phase,indicative_price,matched_volume,matched_amount,unmatched_volume,unmatched_side,source,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(asset_type,symbol,trade_date,observed_at,phase) DO UPDATE SET indicative_price=excluded.indicative_price,matched_volume=excluded.matched_volume,matched_amount=excluded.matched_amount,updated_at=excluded.updated_at",
                                (
                                    asset_type,
                                    code,
                                    day,
                ...
                result["errors"].append(
                    {"provider": "cache", "code": "cache_write_failed", "message": str(exc)}
                )
            return result
        return self.http.chain([("local-auction-cache", cached)], empty)
