...
from .common import MarketDataError, now, require_date, timestamp
...
def dispatch(service, request: Request, body: dict | None):
    ...
    if path.endswith("/kline"):
        ...
        return [{"day": row["time"][:10], **{key: str(row[key]) for key in ("open", "close", "high", "low", "volume")}} for row in rows]
    ...
    if path.endswith("/money-rank"):
        return service.money_rank(q.get("category", "0"), q.get("sort", "netamount"), stocks="/stocks/" in path)
    ...
    if path == "/api/v1/market/sentiment/weighted":
        ...
        return service.sentiment_weighted(body["text"])
    ...
    if path == "/api/v1/market/fund-flows":
        return service.fund_flows(q.get("scope", ""), require_date(q.get("date", "")), q.get("sort", "netamount").lower(), bounded(q, "limit", 20, 1, 100))
    ...
    if path == "/api/v1/market/hot/words":
        return service.hot_words(bounded(q, "hours", 24, 1, 72), bounded(q, "baselineDays", 7, 3, 30), bounded(q, "limit", 30, 1, 100))
    if path.endswith("/auction"):
        return service.auction(code, q.get("assetType", "stock"), require_date(q.get("date", "")))
    if path.endswith("/trades"):
        return service.trades(code, q.get("assetType", "stock"), require_date(q.get("date", "")), bounded(q, "cursor", 0, 0, 1000000), bounded(q, "limit", 100, 1, 500))
    if path.endswith("/chart"):
        end = q.get("to")
        if end and len(end) == 10:
            end = timestamp(end).replace(hour=23, minute=59, second=59, microsecond=999999)
        return service.chart(code, q.get("assetType", "stock"), q.get("market", ""), q.get("period", "day"),
                             q.get("adjustment", ""), q.get("from"), end, bounded(q, "limit", 500, 1, 5000))
    if path.endswith("/drawings"):
        ...
        return service.drawings(code, values.get("assetType", "stock"), values.get("market", ""), values["period"], values["adjustment"],
                                expected_revision=revision, drawings=body.get("drawings"), delete=request.method == "DELETE")
    if path.startswith("/api/v1/themes"):
        return service.theme_api(p.get("id"), "snapshots" if path.endswith("/daily-snapshots") else "catalysts" if path.endswith("/catalysts") else "detail" if p.get("id") else "list", dict(q))
...
GET_PATHS = (
    "stocks/search", "stocks/{code}/snapshot", "stocks/{code}/kline", "stocks/{code}/minute-line",
    "market/telegraphs", "market/indexes/global", "market/industries/rank", "market/industries/money-rank",
    "market/stocks/money-rank", "market/stocks/{code}/money-trend", "market/long-tiger",
    "market/stocks/research-reports", "market/stocks/{code}/research-reports", "market/stocks/notices",
    "market/stocks/{code}/notices", "market/industries/research-reports", "market/industries/{code}/research-reports",
    "market/dictionary", "market/hot/stocks", "market/hot/events", "market/hot/topics", "market/hot/words",
    "market/calendars/investment", "market/calendars/cls", "market/breadth", "market/fund-flows",
    "market/fund-flows/{code}/timeline", "market/futures/positions", "market/margin", "instruments/{code}/auction",
    "instruments/{code}/trades", "instruments/{code}/chart", "instruments/{code}/drawings", "themes",
    "themes/{id}", "themes/{id}/daily-snapshots", "themes/{id}/catalysts", "funds/search", "funds/rankings",
    "etfs/rankings", "etfs/search", "etfs/{code}",
)
...
def create_router(config=None):
    ...
    async def endpoint(request: Request):
        try:
            ...
            value = await run_in_threadpool(dispatch, request.app.state.market, request, body)
    ...
    for path in GET_PATHS:
        router.add_api_route("/api/v1/"+path, endpoint, methods=["GET"], name="market_"+path.replace("/", "_"))
    for method, path in (("POST", "stocks/query"), ("POST", "market/telegraphs/refresh"), ("POST", "market/sentiment/weighted"),
                         ("PUT", "instruments/{code}/drawings"), ("DELETE", "instruments/{code}/drawings")):
        router.add_api_route("/api/v1/"+path, endpoint, methods=[method], name="market_"+method+"_"+path.replace("/", "_"))
    return router
