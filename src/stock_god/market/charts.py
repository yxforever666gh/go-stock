...
from .common import CN, MarketDataError, database_rows, envelope, instrument, now, number, security_id, timestamp
from .news import array

PERIODS = {"1m": 1, "5m": 5, "15m": 15, "30m": 30, "60m": 60,
           "day": 1, "week": 6, "month": 23, "quarter": 66, "year": 260}
...
def aggregate(rows, period):
    ...
    for row in rows:
        ...
        if key not in groups:
            ...
            result = groups[key]
            result.update(high=max(result["high"], row["high"]), low=min(result["low"], row["low"]), close=row["close"],
                          volume=result["volume"]+row["volume"], amount=result["amount"]+row["amount"])
...
class Charts:
    def cached_bars(self, code, start, end, period="1m", adjustment="none"):
        ...
        return aggregate(self._cached_minute_bars(instrument(code)["code"], timestamp(start), timestamp(end)), period)

    def minute_line(self, code, name=""):
        code = instrument(code)["code"]
        endpoint = "https://web.ifzq.gtimg.cn/appstock/app/UsMinute/query" if code.startswith("gb_") else "https://web.ifzq.gtimg.cn/appstock/app/minute/query"
        ...
        for line in payload["data"]:
            ...
            rows.append({"time": fields[0][:2]+":"+fields[0][2:4], "price": number(fields[1]), "volume": number(fields[2]),
    ...
    def _tencent_bars(self, code, start, end, period, adjustment, limit):
        ...
        url = "https://ifzq.gtimg.cn/appstock/app/kline/mkline" if minute else "https://web.ifzq.gtimg.cn/appstock/app/fqkline/get"
        ...
        for row in rows:
            if len(row) < 6:
                continue
            at = datetime.strptime(str(row[0]), "%Y%m%d%H%M").replace(tzinfo=CN) if minute else timestamp(row[0])
            result.append({"time": at.isoformat(), **{field: number(row[index], 0) for field, index in
                          {"open": 1, "close": 2, "high": 3, "low": 4, "volume": 5}.items()},
                          "amount": 0.0, "source": "tencent:"+("none" if minute else adjustment)})
        return valid_bars(result, start, end)

    def _eastmoney_bars(self, code, start, end, period, adjustment, limit):
        minute = period.endswith("m")
        payload = self.http.json("https://push2his.eastmoney.com/api/qt/stock/kline/get",
            {"secid": security_id(code), "klt": "1" if minute else "101", "fqt": {"none": 0, "qfq": 1, "hfq": 2}[adjustment],
             "beg": start.strftime("%Y%m%d"), "end": end.strftime("%Y%m%d"), "lmt": min(limit*PERIODS[period], 350000),
        ...
        for value in array(payload, "data", "klines"):
            ...
            if len(row) < 7:
                continue
            result.append({"time": timestamp(row[0]).isoformat(), **{field: number(row[index], 0) for field, index in
                          {"open": 1, "close": 2, "high": 3, "low": 4, "volume": 5, "amount": 6}.items()},
    ...
    def _sina_bars(self, code, start, end, period, adjustment, limit):
        ...
        rows = array(self.http.json("https://quotes.sina.cn/cn/api/json_v2.php/CN_MarketDataService.getKLineData",
            {"symbol": code, "scale": 1 if period.endswith("m") else 240, "ma": "no", "datalen": min(limit*PERIODS[period], 12000)}))
        result = [{"time": timestamp(row["day"]).isoformat(), **{key: number(row.get(key), 0) for key in
                   ("open", "high", "low", "close", "volume", "amount")}, "source": "sina:none"} for row in rows]
        return valid_bars(result, start, end)

    def _cached_minute_bars(self, code, start, end):
        symbol = code[2:]+"."+code[:2].upper()
        rows = database_rows(self.config.minute_db,
        ...
        result = [{"time": timestamp(row["trade_time"]).isoformat(),
                   **{key: float(row[key] or 0) for key in ("open", "high", "low", "close", "volume", "amount")},
                   "source": row["source"]} for row in rows if row.get("source") and "qfq" not in row["source"] and "hfq" not in row["source"]]
        return valid_bars(result, start, end)
    ...
    def _save_minute_bars(self, code, rows):
        ...
        try:
            with sqlite3.connect(self.config.minute_db, timeout=10) as connection:
                connection.executemany("INSERT INTO minute_bar(stock_code,trade_time,open,high,low,close,volume,amount,source,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?) ON CONFLICT(stock_code,trade_time) DO UPDATE SET open=excluded.open,high=excluded.high,low=excluded.low,close=excluded.close,volume=excluded.volume,amount=excluded.amount,source=excluded.source,updated_at=excluded.updated_at",
                    [(symbol, int(timestamp(row["time"]).timestamp()*1000), *(row[key] for key in ("open", "high", "low", "close", "volume", "amount")),
                      row["source"], int(now().timestamp()*1000)) for row in rows])
    ...
    def _private_bars(self, code, start, end):
        ...
        for page in range(1, 51):
            payload = self.http.json(base+"/stock/history", method="POST",
                headers={"apiKey": self.settings.get("privateMinuteApiKey", "")},
                body={"stock_code": code[2:], "level": "1min", "start_time": start.strftime("%Y-%m-%d %H:%M:%S"),
                      "end_time": end.strftime("%Y-%m-%d %H:%M:%S"), "page": page, "page_size": 5000})
            ...
            for row in items:
                result.append({"time": timestamp(row["trade_time"]).isoformat(),
                    **{key: number(row.get(key), 0) for key in ("open", "high", "low", "close", "amount")},
                    "volume": number(row.get("vol"), 0), "source": "private-minute:none"})
    ...
    def _bars_with_source(self, code, start, end, period, adjustment, limit):
        ...
        if not minute or adjustment == "none":
            sources.append(("tencent", lambda: self._tencent_bars(code, start, end, period, adjustment, limit)))
        if adjustment == "none":
            sources.append(("sina", lambda: self._sina_bars(code, start, end, period, adjustment, limit)))
        sources.append(("eastmoney", lambda: self._eastmoney_bars(code, start, end, period, adjustment, limit)))
        for source, load in sources:
            try:
                ...
                if source == "local-minute-cache":
                    ...
                    if timestamp(rows[0]["time"]) > effective_start+timedelta(minutes=1) or timestamp(rows[-1]["time"]) < effective_end-timedelta(minutes=1):
                    ...
                    try:
                        ...
                        failures.append({"provider": "local-minute-cache", "code": "cache_write_failed", "message": str(exc)})
    ...
    def price_at(self, code, at, sell=False):
        ...
        if abs((current-at).total_seconds()) <= 180:
            ...
            if quoted < at or quoted > current+timedelta(seconds=5) or current-quoted > timedelta(minutes=3):
        ...
        bar = bars[0]
        return {"code": instrument(code)["code"], "price": bar["open"], "asOf": bar["time"], "quoteAt": bar["time"],
                "source": bar["source"], "status": "ok", "mode": "historical_minute"}
    ...
    def daily_closes(self, code, start, end):
        ...
        return [{"tradingDate": row["time"][:10], "close": row["close"], "source": row["source"]} for row in rows]
    ...
    def buy_day_data(self, recommendation):
        ...
        bars = self.bars(code, day.replace(hour=9, minute=30, second=0, microsecond=0),
                         day.replace(hour=15, minute=0, second=0, microsecond=0), "1m", "none")
        result = {"previousClose": close, "bars": bars, "limitRate": 0., "noLimitReason": "",
                  "sourceStatusJson": json.dumps({"sources": sorted({row["source"] for row in bars}), "daily": daily, "errors": []})}
        if len(daily) < 6:
            ...
            result["limitRate"] = .2 if normalized.startswith(("sh68", "sz30")) else .05 if "ST" in recommendation.get("stockName", "").upper() else .1
        return result

    def chart(self, code, asset_type="stock", market="", period="day", adjustment="", start=None, end=None, limit=500):
        ...
        start = timestamp(start) if start else end-timedelta(minutes=limit*PERIODS[period]*3) if period.endswith("m") else end-timedelta(days=limit*PERIODS[period]*2)
        if start > end:
            raise ValueError("from must not be after to")
        data = {"instrument": identity, "period": period, "adjustment": adjustment, "timezone": "Asia/Shanghai",
                "rangeFrom": start.isoformat(), "rangeTo": end.isoformat(), "bars": [], "missingIntervals": []}
        try:
            rows, source, errors = self._bars_with_source(code, start, end, period, adjustment, limit)
            data["bars"] = rows
            return envelope(data, source, as_of=rows[-1]["time"], errors=errors, status="partial" if errors else "ok")
        except MarketDataError as exc:
            data["missingIntervals"] = [{"from": start.isoformat(), "to": end.isoformat(), "reason": str(exc)}]
            return envelope(data, "", status="unavailable", errors=[{"provider": "chart", "code": "unavailable", "message": str(exc)}])

    def drawings(self, code, asset_type, market, period, adjustment, *, expected_revision=None, drawings=None, delete=False):
        identity = instrument(code, asset_type, market)
        ...
        if expected_revision is None:
            rows = database_rows(self.config.main_db, "SELECT * FROM chart_drawing_documents WHERE "+where, key)
            ...
            if not delete:
                ...
                for item in drawings:
                    if not isinstance(item, dict) or not item.get("id") or len(item["id"].encode()) > 128 or item["id"] in ids or item.get("type") not in {"measure", "trend_line", "ray", "fibonacci_retracement", "horizontal_line", "wave"}:
                        raise ValueError("invalid or duplicate drawing")
                    ids.add(item["id"])
                    minimum, maximum = (1, 1) if item["type"] == "horizontal_line" else (3, 64) if item["type"] == "wave" else (2, 2)
            ...
            with sqlite3.connect(self.config.main_db, timeout=10) as connection:
                ...
                current = connection.execute("SELECT * FROM chart_drawing_documents WHERE "+where, key).fetchone()
                ...
                if current:
                    connection.execute("UPDATE chart_drawing_documents SET revision=?,drawings_json=?,deleted_at=?,updated_at=? WHERE "+where,
                                       (revision, payload, deleted, at)+key)
                else:
                    connection.execute("INSERT INTO chart_drawing_documents(drawing_document_id,scope_type,scope_id,asset_type,market,code,period,adjustment,revision,drawings_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                                       (doc_id,)+key+(revision, payload, at, at))
                connection.execute("INSERT INTO chart_drawing_revisions(document_id,revision,drawings_json,deleted_at,created_at) VALUES (?,?,?,?,?)",
                                   (doc_id, revision, payload, deleted, at))
                row = dict(connection.execute("SELECT * FROM chart_drawing_documents WHERE "+where, key).fetchone())
        return {"instrument": identity, "period": period, "adjustment": adjustment, "revision": row["revision"] if row else 0,
                "drawings": json.loads(row["drawings_json"]) if row else [], "updatedAt": row["updated_at"] if row else "0001-01-01T00:00:00Z",
