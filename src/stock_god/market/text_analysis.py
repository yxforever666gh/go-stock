...
ALIASES = {"ai": "人工智能", "a.i": "人工智能", "新能源车": "新能源汽车", "新能源车辆": "新能源汽车",
           "a股市场": "A股", "沪深股市": "A股", "chat gpt": "ChatGPT"}


def calculate_score(tokens):
    score, positive, negative = 0., 0, 0
...
def analyze_tokens(tokens):
    ...
    return {"Score": score, "Category": category, "PositiveCount": positive, "NegativeCount": negative,
    ...
    for row in rows:
        ...
        if found is None:
            for index, previous in enumerate(documents):
                if abs((at-previous["at"]).total_seconds()) <= 21600 and .85 <= len(text)/len(previous["text"]) <= 1.18 and (hashed ^ previous["hash"]).bit_count() <= 3:
                    found = index
                    break
        if found is None:
            indexes[text] = len(documents)
            documents.append({"row": row, "text": text, "hash": hashed, "at": at, "sources": {row.get("source", "")}})
        else:
            indexes[text] = found
            document = documents[found]
            document["sources"].add(row.get("source", ""))
            old = document["row"]
            if (bool(row.get("is_red")), bool(row.get("title")), bool(row.get("url")), at) > (bool(old.get("is_red")), bool(old.get("title")), bool(old.get("url")), document["at"]):
...
class TextAnalysis:
    def _analyzers(self):
        ...
        for tokenizer in (sentiment, hot):
            tokenizer.tmp_dir = str(cache)
            tokenizer.initialize()
        explicit = {}
        for file in (RESOURCES/"finance.txt", self.config.root/"data/dict/user.txt", self.config.root/"runtime/dict/user.txt"):
            if not file.is_file():
                continue
    ...
    def sentiment_weighted(self, text):
        ...
        frequencies = [{"Word": word, "Frequency": count, "Weight": tokenizer.FREQ.get(word, 0), "Score": count*tokenizer.FREQ.get(word, 0)}
                       for word, count in Counter(tokens).items() if tokenizer.FREQ.get(word, 0) >= 100 and any(char.isalnum() for char in word)]
    ...
    def hot_words(self, hours=24, baseline_days=7, limit=30):
        def load():
            ...
            rows = rows[:50000]
            current = dedupe([row for row in rows if timestamp(row.get("data_time") or row["created_at"]) >= begin])
            baseline = dedupe([row for row in rows if timestamp(row.get("data_time") or row["created_at"]) < begin])
            _, tokenizer, explicit = self._analyzers()
            stop = {word.strip().lower() for word in (RESOURCES/"stop_words.txt").read_text(encoding="utf-8").splitlines()}
            tagger = jieba.posseg.POSTokenizer(tokenizer)
            def stats(documents):
                values, positive, negative = {}, 0, 0
                for document in documents:
                    text = (document["row"].get("title") or "")+"\n"+(document["row"].get("content") or "")
                    terms = Counter()
                    for pair in tagger.cut(unicodedata.normalize("NFKC", text), HMM=True):
                        word = ALIASES.get(pair.word.lower(), pair.word).strip()
                        if not 2 <= len(word) <= (24 if word.lower() in explicit else 12 if any("\u4e00" <= char <= "\u9fff" for char in word) else 24):
                            continue
                        if word.lower() in stop or word.lower().startswith(("http", "www")) or not any(char.isalpha() for char in word):
                            continue
                        if word.lower() not in explicit and not (pair.flag.startswith("n") or pair.flag == "eng"):
                            continue
                    ...
                    for word, count in terms.items():
                        value = values.setdefault(word, {"count": 0, "occurrences": 0, "recency": 0., "documents": [], "sources": set()})
                        value["count"] += 1
                        value["occurrences"] += count
                        value["recency"] += math.exp(-math.log(2)*max(0, (end-document["at"]).total_seconds()/3600)/12)
            ...
            for word, value in current_stats.items():
                ...
                ratio = min(20, ((count+.5)/(len(current)+1))/((before+.5)/(len(baseline)+1))) if available else None
                score = math.log1p(count)*(.5+.5*value["recency"]/count)
                if ratio is not None:
                    score *= 1+math.log1p(max(0, ratio-1))
                examples = sorted(value["documents"], key=lambda doc: (bool(doc["row"].get("is_red")), doc["at"]), reverse=True)[:3]
                sources = sorted(value["sources"])
                items.append({"word": word, "score": round(score, 6), "documentCount": count, "occurrenceCount": value["occurrences"],
                    "documentShare": round(count/len(current), 6), "baselineDocumentCount": before, "burstRatio": ratio,
                    "growthPct": (ratio-1)*100 if ratio is not None else None, "sourceCount": len(sources), "sources": sources,
                    "latestAt": max(doc["at"] for doc in value["documents"]).isoformat(),
                    "confidence": "high" if available and count>=5 and len(sources)>=2 else "medium" if count>=3 else "low",
                    "representativeNews": [{"id": doc["row"]["id"], "title": doc["row"].get("title", ""),
                        "excerpt": (doc["row"].get("content") or "")[:180], "source": doc["row"].get("source", ""),
                        "publishedAt": doc["at"].isoformat(), "url": doc["row"].get("url", "")} for doc in examples]})
            ...
            data = {"window": {"from": begin.isoformat(), "to": end.isoformat(), "hours": hours},
                    "baseline": {"from": baseline_start.isoformat(), "to": begin.isoformat(), "requestedDays": baseline_days,
                        "effectiveDays": effective_days, "documentCount": len(baseline), "available": available,
                        "mode": "burst" if available else "coverage_fallback"}, "currentDocumentCount": len(current),
                    "sentiment": {"score": score, "category": category, "description": ("看涨", "看跌", "中性")[category],
                                  "positiveCount": positive, "negativeCount": negative}, "items": items}
            result = envelope(data, "market_news", as_of=max((doc["at"] for doc in current), default=None),
