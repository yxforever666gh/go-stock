...
### 报告问题
如果你发现了一个 bug、有功能请求或者对项目有任何建议，请在项目的 [GitHub Issues](https://github.com/yxforever666gh/go-stock/issues) 中创建一个新的 issue。在创建 issue 时，请提供尽可能多的信息，包括：
...
```bash
git clone https://github.com/yxforever666gh/go-stock.git
cd go-stock
```
...
#### 7. 同步上游仓库
在推送代码之前，确保你的分支与当前公开仓库保持同步；如果你正在维护和原始项目的长期分叉关系，请按你自己的协作流程额外配置上游仓库。
```bash
git remote add upstream https://github.com/yxforever666gh/go-stock.git
git fetch upstream
