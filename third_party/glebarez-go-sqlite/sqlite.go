...
// Backup manages an online SQLite backup created with sqlite3_backup_init.
// This API is backported from modernc.org/sqlite so callers can take a
// transactionally consistent snapshot without changing legacy scan behavior.
type Backup struct {
	srcConn *conn
	dstConn *conn
	pBackup uintptr
...
// NewBackup creates an online backup from the current connection to dstURI.
...
func (c *conn) backup(dstConn *conn) (*Backup, error) {
	...
	return &Backup{srcConn: c, dstConn: dstConn, pBackup: pBackup}, nil
}

// Step copies up to n pages and reports whether more pages remain.
...
// Finish releases the backup handle and closes the destination connection.
