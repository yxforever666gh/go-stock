...
func TestEmptyDatabasesUpgradeDirectlyToSchema13(t *testing.T) {
	...
	if mainStatus.CurrentVersion != 13 || minuteStatus.CurrentVersion != 2 {
		t.Fatalf("schema versions main=%d minute=%d", mainStatus.CurrentVersion, minuteStatus.CurrentVersion)
	}
}

func TestPublished15BaselineUpgradesDirectlyToSchema13(t *testing.T) {
	database := openMigrationTestDB(t)
	...
	if status.CurrentVersion != 13 || len(status.Pending) != 0 {
