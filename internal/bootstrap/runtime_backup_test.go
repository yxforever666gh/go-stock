...
func TestBackupMainBeforePendingMigrationSnapshotsExistingDatabase(t *testing.T) {
	...
	if err := database.Where("id = ?", 22).Delete(&migrations.MigrationRecord{}).Error; err != nil {
		t.Fatal(err)
	}
	...
	if err := backup.Model(&migrations.MigrationRecord{}).Count(&recordCount).Error; err != nil || recordCount != 21 {
