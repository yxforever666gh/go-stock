...
电脑或隧道重启后，先打开本机 7890 代理，再按[数据接口说明](../data-apis.md)启动 Python 隧道服务。等待连接成功后，从 `runtime/minute-api/mcp-url.txt` 复制本次完整 `/mcp` 地址，重新填写到 ChatGPT（认证选 None）；旧地址不会自动更新。更新工具定义后也需要重新扫描工具。

[完整启动、停止和重启步骤](../data-apis.md)。当前没有替用户完成 ChatGPT 账号内的添加和扫描；账号是否允许文件下载工具须在实际接入时确认。
...
实际抽样状态见 [蝶梦 MCP 验证记录](../history/蝶梦MCP验证记录.md)。模拟测试通过不等于全部接口均已联网验证，空结果也不保证该日期有数据。
