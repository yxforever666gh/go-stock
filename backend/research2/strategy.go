...
const CurrentStrategyVersion = "research2-slots-v11"
...
// preparedEvidence belongs to one run. Every consumer uses the same score
// support, derived only after both snapshot and catalyst windows are known.
type preparedEvidence struct {
	Evidence
	scoreEvidence         map[string]CandidateScoreEvidence
	catalystWindowStartAt time.Time
	knowledgePrompt       string
}

func prepareEvidence(evidence Evidence, catalystWindowStartAt time.Time) preparedEvidence {
	if evidence.FreezeAt.IsZero() {
		evidence.FreezeAt = evidence.CutoffAt
	}
	prepared := preparedEvidence{Evidence: evidence, catalystWindowStartAt: catalystWindowStartAt, scoreEvidence: make(map[string]CandidateScoreEvidence, len(evidence.Candidates))}
	for _, candidate := range evidence.Candidates {
		code, ok := trading.NormalizeMainlandCode(candidate.Code)
		if !ok {
			continue
		}
		if _, exists := prepared.scoreEvidence[code]; !exists {
			prepared.scoreEvidence[code] = buildCandidateScoreEvidence(code, evidence.Documents, evidence.CutoffAt, evidence.FreezeAt, catalystWindowStartAt)
		}
	}
	return prepared
}
...
type EvidenceCollector interface {
	Collect(context.Context, time.Time, float64) (Evidence, error)
}

type RunEvidenceCollector interface {
	CollectForRun(context.Context, string, time.Time, float64) (Evidence, error)
}

type FilteredRunEvidenceCollector interface {
	CollectForRunWithExclusions(context.Context, string, time.Time, map[string]struct{}, float64) (Evidence, error)
}
...
func (r *Runner) run(ctx context.Context, scheduledFor time.Time, triggerSource, chainID, parentRunID string) (AnalysisRun, error) {
	...
	run := AnalysisRun{ScheduledSlot: normalizeSlot(SlotAt(scheduledFor)), RunID: uuid.NewString(), TradingDate: tradingDate, ScheduledFor: scheduledFor, StartedAt: now, EvidenceCutoffAt: cutoff, EvidenceWindowStartAt: &windowStart, StrategyVersion: CurrentStrategyVersion, Status: "running", SourceStatusJSON: "[]", ModelAttemptLogJSON: "[]", ChainID: chainID, ParentRunID: parentRunID, TriggerSource: triggerSource}
	...
	finishFailure := func(status, reason string, cause error) (AnalysisRun, error) {
		...
		run.OnTime = SlotAt(completed) == run.ScheduledSlot
		if saveErr := r.repository.SaveRun(ctx, &run); saveErr != nil {
	...
	// Analysis is account-neutral: the destination account is only known at
	// publication. Affordability is checked again by the destination buyer.
	availableCash := math.MaxFloat64
	excludedCodes := map[string]struct{}{}
	var collected Evidence
	if availableCash == 0 {
		collected = Evidence{CutoffAt: cutoff, SourceStatusJSON: "[]"}
	} else if collector, ok := r.collector.(FilteredRunEvidenceCollector); ok {
		collected, err = collector.CollectForRunWithExclusions(ctx, run.RunID, cutoff, excludedCodes, availableCash)
	} else if collector, ok := r.collector.(RunEvidenceCollector); ok {
		collected, err = collector.CollectForRun(ctx, run.RunID, cutoff, availableCash)
	} else {
		collected, err = r.collector.Collect(ctx, cutoff, availableCash)
	}
	collected.AvailableCash = availableCash
	var sourceStates []map[string]any
	_ = json.Unmarshal([]byte(defaultJSON(collected.SourceStatusJSON, "[]")), &sourceStates)
	sourceStates = append(sourceStates, map[string]any{"sourceId": "research2:cash", "sourceName": "分析阶段不按账户现金过滤", "status": "ok", "accountNeutral": true, "collectedAt": now})
	cashAudit, _ := json.Marshal(sourceStates)
	run.SourceStatusJSON = string(cashAudit)
	if !collected.CutoffAt.IsZero() {
		cutoff = collected.CutoffAt.In(shanghai())
		run.EvidenceCutoffAt = cutoff
	} else {
		collected.CutoffAt = cutoff
	}
	if !collected.WindowStartAt.IsZero() {
		windowStart = collected.WindowStartAt.In(shanghai())
		collected.WindowStartAt = windowStart
	...
	coverage := collected.CoveragePct
	run.EvidenceCoveragePct = &coverage
	degraded := collected.Degraded
	run.Degraded = &degraded
	if collected.EvidenceSetID != "" {
		run.EvidenceProfileVersion, run.EvidenceSetID = collected.EvidenceProfileVersion, collected.EvidenceSetID
	}
	...
	evidence := prepareEvidence(collected, previousCatalystSessionClose(ctx, r.calendar, cutoff))
	if r.knowledge != nil {
		...
		if retrievalErr == nil && strings.TrimSpace(retrieval.Prompt) != "" {
			evidence.knowledgePrompt = retrieval.Prompt
		}
	}
	...
	output := modelOutput{Conclusion: "没有满足当前可用现金和数据约束的可评分标的。"}
	var sourceValidationMessages []string
	for structureAttempt := 1; len(evidence.Candidates) > 0 && structureAttempt <= 2; structureAttempt++ {
		...
		if err == nil {
			sourceValidationMessages = validateModelSourceRefs(output.Recommendations, evidence)
		}
	...
	run.OnTime = SlotAt(generated) == run.ScheduledSlot
	for index := range items {
		...
		if status == "analysis_only" {
			items[index].FailureReason = "报告在上午窗口外完成，仅保存分析，不进入模拟交易及收益统计"
		}
	}
	...
	if err = func() error {
		...
		repository.now = r.now
		return repository.FinalizeRun(ctx, &run, items, func() string {
...
func assignResearch2SelectionRanks(items []Recommendation) {
	for index := range items {
		items[index].SelectionRank = index + 1
		items[index].SelectionRole = ""
		items[index].Status = "buy_pending"
	}
}

func research2AuditEvidence(evidence preparedEvidence) map[string]any {
	return map[string]any{
		...
		"catalystWindowStartAt": evidence.catalystWindowStartAt,
	}
}

func (r *Runner) recordResearch2Attempts(ctx context.Context, runID, phase string, callSequence int, cutoff time.Time, prompt string, evidence preparedEvidence, attempts []aicontract.ModelAttemptRecord, result aicontract.CompletionResult, repaired bool, callErr error) error {
...
func buildPrompt(evidence preparedEvidence, cutoff time.Time) string {
	...
	scoreEvidenceJSON, _ := json.Marshal(evidence.scoreEvidence)
	return strings.Join([]string{
		strategyPrompt,
		"\n# 本次执行参数",
		"- 策略版本：" + CurrentStrategyVersion,
		...
		"- 本轮分析不预先按账户现金或持仓排除候选；成功落盘后，由实际归属账户按可用现金和评分顺序买入，最多五只，不借款。",
		...
		"\n# 本轮候选评分依据（按规范化股票代码索引）",
		string(scoreEvidenceJSON),
		"\n# 知识库参考（不是市场评分来源）",
		strings.TrimSpace(evidence.knowledgePrompt),
	}, "\n")
}
...
func validateRecommendations(runID string, generated time.Time, evidence preparedEvidence, values []modelRecommendation) ([]Recommendation, []string) {
	items := make([]Recommendation, 0, len(values))
	warnings := make([]string, 0)
	seen := map[string]struct{}{}
	allowed := make(map[string]string, len(evidence.Candidates))
	for _, candidate := range evidence.Candidates {
	...
	for _, value := range values {
		...
		if sourceWarnings := validateRecommendationSourceRefs(code, value.SourceRefs, evidence.Documents, evidence.FreezeAt, evidence.Candidates); len(sourceWarnings) > 0 {
			warnings = append(warnings, sourceWarnings...)
			continue
		}
		if scoreWarnings := validateRecommendationScoreEvidence(code, value, evidence); len(scoreWarnings) > 0 {
		...
		if evidence.CandidateReferencePrices != nil {
			...
			referencePrice, priceExists = evidence.CandidateReferencePrices[code]
			if !priceExists {
...
func validateRecommendationScoreEvidence(code string, value modelRecommendation, evidence preparedEvidence) []string {
	byID := make(map[string]researchevidence.SourceDocument, len(evidence.Documents))
	...
	for _, ref := range value.SourceRefs {
		...
		if !ok || document.AvailableAt == nil || document.AvailableAt.After(evidence.FreezeAt) || strings.TrimSpace(document.Error) != "" || !research2CitationPayloadUsable(document.Content) || !scoreScopedDocumentApplies(document, code, evidence.Candidates, evidence.Documents, evidence.FreezeAt) {
			continue
		}
	...
	proof := evidence.scoreEvidence[code]
...
func renderAnalysisReport(run AnalysisRun, evidence preparedEvidence, output modelOutput, items []Recommendation, warnings []string) string {
	...
	lines := []string{
		...
		"- 计划区间：" + run.ScheduledSlot,
		"- 落盘区间：" + defaultReportText(run.Slot),
	...
	if len(items) > 0 {
		lines = append(lines,
			"", "## 候选评分与执行安排", "",
			"| 排名 | 代码 | 名称 | 最终分 | 参考价 | 100股预计成本 | 市场/板块/个股/催化 | 风险扣分 | 执行安排 |",
			"|---:|---|---|---:|---:|---:|---|---:|---|",
		)
		for _, item := range items {
			...
			lines = append(lines, fmt.Sprintf("| %d | %s | %s | %.2f | %.3f | %.2f | %.1f / %.1f / %.1f / %.1f | %.1f | %s |", item.SelectionRank, item.StockCode, escapeMarkdownCell(item.StockName), item.FinalScore, item.ReferencePrice, item.EstimatedLotCost, item.MarketScore, item.SectorScore, item.StockScore, item.CatalystScore, item.RiskDeduction, execution))
		}
		for _, item := range items {
			lines = append(lines,
	...
	lines = append(lines, "", "评分范围说明：保存通过校验的本轮候选评分，缺失或无效评分见数据校验，不代表全市场。不设最低分门槛，按评分顺序和所属账户当前现金每天最多成交五只；未买入股票不参与成交收益。相同市场分是正常现象，缺失分项不折算满分，也不为补位调分。")
	if len(warnings) > 0 {
...
func recommendationExecution(generated, tradingDay time.Time) (time.Time, bool, string) {
	if SlotAt(generated) == "" || generated.In(shanghai()).Format("2006-01-02") != tradingDay.In(shanghai()).Format("2006-01-02") {
		return generated, true, "analysis_only"
	}
	return generated, false, "buy_pending"
}
...
func (s *TradingService) ProcessDue(ctx context.Context, now time.Time) error {
	...
	defer s.mu.Unlock()
	now = now.In(shanghai())
	open, err := s.calendar.IsTradingDay(ctx, now)
	if err != nil {
		return err
	}
	if !open {
...
func (s *TradingService) processBuys(ctx context.Context, now time.Time) error {
	...
	for _, runID := range order {
		...
		repository := s.repository.WithSlot(run.Slot)
		remainingSlots, countErr := repository.RemainingBuySlots(ctx, now)
		if countErr != nil {
			return countErr
		}
		...
		if remainingSlots == 0 {
			for _, item := range group {
				if err := s.repository.MarkStatus(ctx, item.RecommendationID, "analysis_only", "当日执行已结束，剩余评分仅保留分析"); err != nil {
					return err
				}
			}
			continue
		...
		for _, item := range group {
			executionCutoff := time.Date(now.Year(), now.Month(), now.Day(), 11, 30, 0, 0, shanghai())
			...
			if strings.TrimSpace(run.ChainID) != "" && !now.Before(executionCutoff) && item.SignalAt.Before(lunchStart) {
				if markErr := s.repository.MarkStatus(ctx, item.RecommendationID, "analysis_only", "上午未完成的买入已到11:30截止，不延迟追单"); markErr != nil {
					return markErr
				}
				continue
			}
			...
			if quoteErr != nil {
				...
				pendingQuotes[item.RecommendationID] = true
			...
			case snapshot.Suspended:
				failureCode, failureReason = "suspended", "停牌，本报告跳过"
			case snapshot.LimitUp:
				failureCode, failureReason = "limit_up", "已涨停，本报告跳过"
		...
		if len(valid) == 0 {
			if run.ChainID != "" {
				if _, err = s.repository.RefreshExecutionChainFilled(ctx, run.ChainID); err != nil {
					return err
				}
			}
			continue
		...
		for _, item := range valid {
			slots, slotErr := repository.RemainingBuySlots(ctx, s.now())
			if slotErr != nil {
				return slotErr
			}
			if slots <= pendingQuoteCount {
				break
			}
			if pendingQuotes[item.RecommendationID] {
				pendingQuoteCount++
				continue
			}
			sellAt, nextErr := s.nextTradingDayAt(ctx, item.TargetBuyAt, SlotTime(now, run.Slot).Hour(), SlotTime(now, run.Slot).Minute())
			if nextErr != nil {
				return nextErr
			}
			...
			overview, overviewErr := repository.Overview(ctx)
			if overviewErr != nil {
				return overviewErr
			}
			slots, slotErr = repository.RemainingBuySlots(ctx, checkedAt)
			if slotErr != nil {
				return slotErr
			}
			if slots == 0 {
			...
			cashCap := math.Min(overview.Cash, math.Max(overview.Cash/float64(slots), lotCost))
			...
			if err = repository.RecordBuy(ctx, item.RecommendationID, trade, sellAt); err != nil {
				if errors.Is(err, trading.ErrInsufficientCash) {
					if markErr := s.repository.MarkStatus(ctx, item.RecommendationID, "missed_cash", "成交事务检查发现当前现金不足"); markErr != nil {
						return errors.Join(err, markErr)
					}
					continue
				}
				if errors.Is(err, trading.ErrNewPositionsDisabled) || errors.Is(err, ErrDailyBuyLimitReached) || errors.Is(err, ErrExecutionChainClosed) || errors.Is(err, errDuplicateBuy) {
		...
		if strings.TrimSpace(run.ChainID) != "" {
			...
			if chain.Status == "completed" {
				for _, item := range group {
					if err := s.repository.FinishPendingBuy(ctx, item.RecommendationID); err != nil {
						return err
					}
				}
			}
	...
	for _, slot := range Slots() {
		if _, err = s.repository.WithSlot(slot).SaveSnapshot(ctx, "trade_cycle", now); err != nil {
			return err
		}
	}
	return nil
...
func (s *TradingService) buyWindowAvailable(ctx context.Context, item Recommendation, chainID string, now time.Time) (bool, error) {
	...
	if chainID != "" && !local.Before(cutoff) && item.SignalAt.Before(lunch) {
		return false, s.repository.MarkStatus(ctx, item.RecommendationID, "analysis_only", "行情返回时已到11:30截止，不延迟追单")
	}
...
func (s *TradingService) processSells(ctx context.Context, now time.Time) error {
	return s.processSlotSells(ctx, now)
}
...
func continuousAuction(value time.Time) bool {
	...
	afternoonOpen := time.Date(local.Year(), local.Month(), local.Day(), 11, 30, 0, 0, shanghai())
