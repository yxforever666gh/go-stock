	...
	return research2TransactionWithWriteRetry(ctx, r.db, func(tx *gorm.DB) error {
		...
		if run.TriggerSource == "diagnostic" {
			...
			if stored.ArchiveReason != "" {
				...
				updates := map[string]any{"winner_run_id": run.RunID, "latest_run_id": run.RunID, "root_run_id": run.RunID, "status": "running", "target_slots": DailyTargetSlots, "allocation_policy": AllocationPolicyRemainingCashSlots}
