...
func resolveLogLevel() (zapcore.Level, bool) {
	level := appconfig.Load().Log.Level
	switch level {
	case "silent":
...
func InitLogger() {
	//获取编码器
	encoder := getEncoder()
	minLevel, enabled := resolveLogLevel()
	...
	lowPriority := zap.LevelEnablerFunc(func(lev zapcore.Level) bool { //info和debug级别,debug级别是最低的
		...
		return lev < zap.ErrorLevel && lev >= minLevel
	})
...
func getInfoWriterSyncer() zapcore.WriteSyncer {
	cfg := appconfig.Load()
	filename := cfg.LogFilePath("info.log")
...
func getErrorWriterSyncer() zapcore.WriteSyncer {
	cfg := appconfig.Load()
	filename := cfg.LogFilePath("error.log")
	_ = os.MkdirAll(filepath.Dir(filename), os.ModePerm)
	//引入第三方库 Lumberjack 加入日志切割功能
