	...
	if fileutil.IsExist("data/dict/user.txt") {
		...
		for _, line := range lines {
			...
			case 2:
				...
				if convErr != nil {
					logger.SugaredLogger.Warnf("跳过用户词典[%s]，词频解析失败: %v", line, convErr)
					continue
				}
				if freq <= 0 {
					logger.SugaredLogger.Warnf("跳过用户词典[%s]，词频必须大于0", line)
					continue
				}
				if ok {
...
func SaveAnalyzeSentimentWithFreqWeight(frequencies []models.WordFreqWithWeight) {
	wordAnalyzes := buildTopWordAnalyzes(frequencies, 10)
...
func buildTopWordAnalyzes(frequencies []models.WordFreqWithWeight, limit int) []models.WordAnalyze {
	if limit <= 0 || len(frequencies) == 0 {
		return nil
	}
	...
	limit = min(len(frequencies), limit)
	wordAnalyzes := make([]models.WordAnalyze, 0, limit)
