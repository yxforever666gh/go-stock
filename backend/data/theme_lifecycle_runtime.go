...
func NewThemeLifecycleRuntime(sources *ThemeSourceAggregator, service *themes.Service, repository *themes.Repository, clock func() time.Time) *ThemeLifecycleRuntime {
	if clock == nil {
		clock = time.Now
	}
...
func groupThemeLifecycleSignals(ctx context.Context, repository *themes.Repository, signals []RawThemeSignal) ([]*themeLifecycleGroup, error) {
	union := newThemeNameUnion()
	for _, signal := range signals {
		primary := themes.NormalizeName(signal.ThemeName)
		if primary == "" {
			continue
		}
		union.add(primary)
		for _, alias := range signal.Aliases {
			normalized := themes.NormalizeName(alias)
			if normalized == "" {
				continue
			}
	...
	for _, group := range initial {
		...
		key := "new:" + themes.NormalizeName(group.CanonicalName)
		for id := range ids {
			group.ThemeID = id
			key = "id:" + id
	...
	for _, group := range merged {
		if group.ThemeID == "" {
			group.ThemeID = "theme-" + themeLifecycleHash(themes.NormalizeName(group.CanonicalName))[:32]
		}
		group.NormalizedName = themes.NormalizeName(group.CanonicalName)
...
func (runtime *ThemeLifecycleRuntime) freezeThemeGroup(ctx context.Context, group *themeLifecycleGroup, tradeDate string, observedAt, frozenAt time.Time) (ThemeLifecycleFreezeResult, error) {
	aliases := make([]themes.AliasInput, 0, len(group.Names))
	for _, name := range sortedThemeLifecycleNames(group.Names) {
		if themes.NormalizeName(name) == themes.NormalizeName(group.CanonicalName) {
			continue
		}
		aliases = append(aliases, themes.AliasInput{
			ID: "alias-" + themeLifecycleHash(group.ThemeID + "|" + themes.NormalizeName(name))[:32], Alias: name, Source: group.Names[name],
		})
	}
	stored, err := runtime.Repository.UpsertTheme(ctx, themes.UpsertThemeRequest{
	...
	request := themes.FreezeSnapshotRequest{
...
func mapThemeLifecycleFreezeResult(name string, snapshot themes.DailySnapshot, existing bool) ThemeLifecycleFreezeResult {
...
func nextThemeLifecycle(previous *themes.DailySnapshot, heat float64, rank, sourceCount int, conflict bool) (int, themes.LifecycleStage) {
	if previous == nil {
		return 1, themes.StageObserve
	}
	cycleNo, stage := previous.CycleNo, previous.LifecycleStage
	switch previous.LifecycleStage {
	case themes.StageObserve:
		if sourceCount >= 2 && heat >= 50 {
			stage = themes.StageFerment
		}
	case themes.StageFerment:
		if heat >= 75 && rank <= 10 {
			stage = themes.StageAccelerate
		}
	case themes.StageAccelerate:
		if conflict || previous.HeatScore-heat >= 15 {
			stage = themes.StageDiverge
		}
	case themes.StageDiverge:
		if heat < 40 {
			stage = themes.StageFade
		}
	case themes.StageFade:
		if heat >= 50 {
			cycleNo++
			stage = themes.StageObserve
		}
	}
...
type themeLifecycleEvent struct {
	...
	Claims      []themes.ClaimInput
}

func makeThemeLifecycleCatalystRequests(themeID string, signals []RawThemeSignal) []themes.IngestCatalystRequest {
	...
	for _, signal := range signals {
		...
		event := events[key]
		if event == nil {
			event = &themeLifecycleEvent{EventType: eventType, Title: strings.TrimSpace(signal.Title), Summary: summary, EventAt: signal.EventAt, EntityKeys: map[string]struct{}{}, Claims: []themes.ClaimInput{}}
			events[key] = event
		} else {
		...
		event.Claims = append(event.Claims, themes.ClaimInput{
	...
	sort.Strings(keys)
	result := make([]themes.IngestCatalystRequest, 0, len(keys))
	for _, key := range keys {
		...
		result = append(result, themes.IngestCatalystRequest{
...
func themeLifecycleClaimsConflict(claims []themes.ClaimInput) bool {
...
func themeLifecycleStoredClaimsConflict(claims []themes.SourceClaim) bool {
	supports, contradicts := false, false
	for _, claim := range claims {
...
func themeLifecycleEventIdentity(signal RawThemeSignal) string {
	return themes.CanonicalEventIdentity(
...
func themeLifecycleConstituents(signals []RawThemeSignal) []themes.SnapshotConstituent {
	...
	result := make([]themes.SnapshotConstituent, 0, len(values))
	for index, value := range values {
		result = append(result, themes.SnapshotConstituent{
...
func summarizeThemeLifecycleSignals(signals []RawThemeSignal) string {
	...
	for _, signal := range values {
		...
		key := themes.NormalizeName(value)
		if value == "" || key == "" {
			continue
		}
		if _, exists := seen[key]; exists {
