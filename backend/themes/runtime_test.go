...
func TestThemeLifecycleRuntimeAdvancesDeterministicStages(t *testing.T) {
	...
	wantStages := []LifecycleStage{
		StageObserve, StageFerment, StageAccelerate,
		StageDiverge, StageFade, StageObserve,
	}
...
func TestNextThemeLifecycleRules(t *testing.T) {
	base := DailySnapshot{CycleNo: 1, Rank: 1, HeatScore: 90}
	tests := []struct {
		name      string
		...
		{name: "new starts observing", heat: 100, rank: 1, sources: 3, wantCycle: 1, wantStage: StageObserve},
		{name: "observe needs independent sources", previous: lifecycleSnapshot(base, StageObserve), heat: 80, rank: 1, sources: 1, wantCycle: 1, wantStage: StageObserve},
		{name: "observe ferments at threshold", previous: lifecycleSnapshot(base, StageObserve), heat: 50, rank: 20, sources: 2, wantCycle: 1, wantStage: StageFerment},
		{name: "ferment rank gate", previous: lifecycleSnapshot(base, StageFerment), heat: 90, rank: 11, sources: 3, wantCycle: 1, wantStage: StageFerment},
		{name: "ferment accelerates", previous: lifecycleSnapshot(base, StageFerment), heat: 75, rank: 10, sources: 2, wantCycle: 1, wantStage: StageAccelerate},
		{name: "accelerate diverges on heat drop", previous: lifecycleSnapshot(base, StageAccelerate), heat: 75, rank: 1, sources: 2, wantCycle: 1, wantStage: StageDiverge},
		{name: "accelerate diverges on conflict", previous: lifecycleSnapshot(base, StageAccelerate), heat: 90, rank: 1, sources: 2, conflict: true, wantCycle: 1, wantStage: StageDiverge},
		{name: "diverge fades below forty", previous: lifecycleSnapshot(base, StageDiverge), heat: 39.99, rank: 1, sources: 2, wantCycle: 1, wantStage: StageFade},
		{name: "fade starts next cycle", previous: lifecycleSnapshot(base, StageFade), heat: 50, rank: 1, sources: 2, wantCycle: 2, wantStage: StageObserve},
	}
...
func TestThemeLifecycleRuntimeSameDayIsIdempotent(t *testing.T) {
	...
	catalysts, err := fixture.repository.ListCatalysts(context.Background(), ListCatalystsRequest{ThemeID: first.Themes[0].ThemeID})
	require.NoError(t, err)
...
func TestThemeLifecycleRuntimeConceptAndFundFlowDoNotCreateCatalysts(t *testing.T) {
	...
	catalysts, err := fixture.repository.ListCatalysts(context.Background(), ListCatalystsRequest{ThemeID: result.Themes[0].ThemeID})
	require.NoError(t, err)
...
func TestThemeLifecycleRuntimeCrossBatchConflictTriggersDivergence(t *testing.T) {
	...
	fixture := newThemeLifecycleRuntimeFixture(t, adapter)
	wantStages := []LifecycleStage{StageObserve, StageFerment, StageAccelerate, StageDiverge}
	var themeID string
	...
	catalysts, err := fixture.repository.ListCatalysts(context.Background(), ListCatalystsRequest{ThemeID: themeID})
	require.NoError(t, err)
	var matched *CatalystEvent
...
func newThemeLifecycleRuntimeFixture(t *testing.T, adapters ...SourceAdapter) *themeLifecycleRuntimeFixture {
	t.Helper()
	repository, _ := newThemeTestRepository(t)
	...
	fixture.runtime = NewThemeLifecycleRuntime(aggregator, NewService(repository), repository, func() time.Time { return fixture.now })
	return fixture
}
...
func lifecycleSnapshot(base DailySnapshot, stage LifecycleStage) *DailySnapshot {
	copy := base
