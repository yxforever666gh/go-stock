...
import {adaptPredictionChart, predictionChartOverlays} from './prediction-chart-adapter.js'
...
test('research adapter preserves real bars, source errors, PnL and missing sessions', () => {
  const model = adaptPredictionChart(chart)
  ...
  const overlays = predictionChartOverlays(model, trades, {showPriceLines: true})
...
test('percentage axis uses the latest visible trading session previous close', () => {
  ...
  assert.equal(predictionChartOverlays(adaptPredictionChart(current)).mainPercentBase, 12.5)
  assert.equal(predictionChartOverlays(adaptPredictionChart({...current, sessions: []})).mainPercentBase, null)
})
...
test('data as-of comes from quotes or bars, never from a later collection time', () => {
  ...
  assert.equal(adaptPredictionChart(current).asOf, chart.bars.at(-1).at)
  assert.equal(adaptPredictionChart({...current, bars: []}).asOf, '')
  assert.equal(adaptPredictionChart({...current, quoteAt: '2026-09-07T10:59:59+08:00'}).asOf, '2026-09-07T10:59:59+08:00')
})
...
test('partial session is not expanded into a false full-day missing interval', () => {
  ...
  const model = adaptPredictionChart(current)
  assert.equal(model.missingIntervals.length, 0)
  assert.match(predictionChartOverlays(model).tooltipLines(model.bars[0])[0], /涨跌幅：--/)
})
