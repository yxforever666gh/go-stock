...
<template>
  <n-space vertical>
    ...
    <n-data-table :columns="columns" :data="rows" :loading="loading" :scroll-x="1340" :row-key="row => row.runId"/>
  </n-space>
  <n-modal v-model:show="visible"><n-card title="隔夜强势分析报告" closable style="width:min(1200px,94vw);max-height:94vh" @close="visible=false"><n-scrollbar style="max-height:82vh"><n-spin :show="!detail"><AppMarkdownPreview v-if="detail" :model-value="detail.reportMarkdown || detail.failureReason || '暂无报告'"/></n-spin></n-scrollbar></n-card></n-modal>
</template>
