...
import {
  CreatePredictionReplay,
  ExportPredictionRunAudit,
  GetPredictionReplay,
  GetPredictionRunAudit,
  ListPredictionReplayModelConfigs,
} from '../../services/prediction-audit-api'
import {
  AUDIT_TABS,
  auditIsAvailable,
  auditIsLegacy,
...
const audit = ref(normalizePredictionAudit(null))
...
async function loadModelConfigs() {
  ...
  try {
    const models = await ListPredictionReplayModelConfigs()
...
async function loadAudit() {
  ...
  loadError.value = ''
  loading.value = false
  audit.value = normalizePredictionAudit(null, {ownerId: props.ownerId})
  activeTab.value = AUDIT_TABS[0].name
  resetReplay()
  if (!props.active || !props.ownerId) return
  loading.value = true
  try {
    const response = await GetPredictionRunAudit(props.ownerId)
    if (requestVersion !== auditRequestVersion) return
    audit.value = normalizePredictionAudit(response, {ownerId: props.ownerId})
...
async function exportAudit() {
  ...
  try {
    const filename = await ExportPredictionRunAudit(props.ownerId)
...
async function refreshReplay() {
  ...
  try {
    const refreshed = await GetPredictionReplay(replayId)
...
async function createReplay() {
  ...
  try {
    const created = await CreatePredictionReplay(props.ownerId, Number(modelConfigId.value))
    if (requestVersion !== replayRequestVersion) return
...
watch(() => [props.ownerId, props.active], loadAudit, {immediate: true})
onBeforeUnmount(() => {
  auditRequestVersion++
  replayRequestVersion++
  modelRequestVersion++
...
<template>
  <n-space vertical size="large" class="prediction-audit-panel">
