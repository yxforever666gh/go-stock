...
test('normalizes available and legacy audit contracts without inventing payloads', () => {
  const available = normalizePredictionAudit({
    availability: 'available', ownerType: 'research2', ownerId: 'run-1',
  ...
  const legacy = normalizePredictionAudit({data: {availability: 'legacy_unavailable'}}, {ownerType: 'research2', ownerId: 'old-run'})
