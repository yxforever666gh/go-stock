...
import {GetResearch2Performance, GetResearch2PortfolioPerformance, GetResearch2Recommendation, ListResearch2PerformanceRecommendations} from '../services/research2-api'
...
import Research2PortfolioReturnChart from './Research2PortfolioReturnChart.vue'
...
const querySlots = String(route.query.performanceSlots || '').split(',').filter(validResearch2Slot)
const selectedSlots = ref(querySlots.length ? [...new Set(querySlots)] : RESEARCH2_SLOTS.map(item => item.value))
...
const slotOptions = RESEARCH2_SLOTS
...
const history = useResearchList(async (limit, offset) => await ListResearch2PerformanceRecommendations(limit, offset, effectiveSlots.value, fromDate.value || '', toDate.value || '') || [])
...
function selectAllSlots() { selectedSlots.value = RESEARCH2_SLOTS.map(item => item.value) }
function syncURL() {
  router.replace({name: 'research2', query: {
    ...route.query,
...
async function refresh() {
  ...
  loading.value = true
  try {
    const requests = [GetResearch2PortfolioPerformance(effectiveSlots.value, fromDate.value || '', toDate.value || ''), history.refresh()]
    if (mode.value === 'individual') requests.push(GetResearch2Performance(props.slot))
...
<template>
  <n-space vertical size="large">
    ...
    <n-card title="收益率日变化" size="small"><Research2PortfolioReturnChart :points="portfolio?.curve || []"/></n-card>
