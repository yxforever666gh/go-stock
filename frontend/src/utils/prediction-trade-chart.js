...
export const PREDICTION_CHART_PRICE_LINES_STORAGE_KEY = 'stock-god.prediction-chart.show-price-lines'

export function readPriceLinesPreference(storage) {
  try {
    return storage?.getItem(PREDICTION_CHART_PRICE_LINES_STORAGE_KEY) === 'true'
  } catch (_) {
    return false
  }
}
...
export function writePriceLinesPreference(storage, value) {
  try {
    storage?.setItem(PREDICTION_CHART_PRICE_LINES_STORAGE_KEY, value ? 'true' : 'false')
  } catch (_) {
