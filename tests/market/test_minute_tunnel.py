...
def test_provisioning_rejects_oversized_or_invalid_responses():
    for body in (b"not-json", b'"' + b"x" * (1 << 20) + b'"'):
        p = Provisioner(
            httpx.Client(transport=httpx.MockTransport(lambda request: httpx.Response(200, content=body)))
        )
        with pytest.raises(RuntimeError):
            p.prepare()
        assert p.prepared is None
...
def test_supervisor_publishes_only_registered_tunnel_and_cleans_owned_group(
    tmp_path, monkeypatch, tunnel_fails
):
    ...
    monkeypatch.setattr(module, "_wait_http", lambda *args, **kwargs: None)
