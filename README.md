...
- 应用层接口已经按业务域拆分到多个 `app_*_api.go` 文件，`app.go` 只保留应用主结构和共享运行时逻辑，后续继续扩展时不再需要维护超大入口文件。
- 前端路由正式拆入 `frontend/src/views/`，页面级入口和业务组件分层更清晰；Wails 绑定统一通过 `frontend/src/services/app-api.js` 暴露，页面侧接入更稳定。
- AI 智能体页面补强为“带历史侧栏的多轮会话”模式，支持会话列表、标题自动总结、消息持久化和轮次导航，连续分析体验更接近真正的工作台。
- 股票页和智能体页的重型逻辑已下沉到 composables，图表渲染、导出与会话流式状态管理不再混在单个大组件里，前端维护成本明显下降。
- 设置页继续收敛到当前真实可维护能力：分钟线来源、公共源开关、私人分钟线参数、邮件发送与 AI 配置形成统一配置入口；通知相关旧能力仍保留兼容，但不再作为主流程维护。
- 根目录新增 `Makefile` 统一工程入口，适合本地开发、脚本调用和 CI 接入；`.gitignore` 也补充了本地工作区、临时数据和私有集成说明的忽略规则。
