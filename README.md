...
研究中心2使用24个独立五分钟账户：交易日09:30至11:25每五分钟定时卖出对应账户旧持仓，同时独立启动研究。成功结果按落盘时间归区间，首份有效报告立即参与买入，每账户每天最多5只；后到及11:30后完成的报告仅归档。每个新分区按本轮报告落盘时的账户现金确定五分之一仓位基准：一手含费成本达到或超过该值时买一手，其余成交严格低于该值，且始终不超过当前可用现金。模拟卖出不受停牌或可卖日期限制，报价失败使用已保存有效价格并标明来源，仍扣除费用。开盘首轮允许少量或零根分钟线，缺失指标不补造。迁移一次性复制原账户加款前现金并给每个账户加10000元，旧股票按信号时间分区且不复制；账户收益从迁移基准重新统计，旧成交及个股持有期收益保留。策略版本为 `research2-slots-v12`，证据版本为 `research2-slots-v8`。历史主备字段保留用于旧报告，待对应旧记录归档后可删除。
