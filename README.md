...
Go-Stock 是基于 Go、Vue 3、Naive UI 和 SQLite 的本地股票行情与 AI 研究工具。本版本保留市场行情、股票自选和基金，并为可审计证据与更细粒度的多资产行情数据建立第一阶段基础设施。
