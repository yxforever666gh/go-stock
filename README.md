...
- 研究中心1：保留原 `AI分析报告`、`股票推荐记录`、`股票收益率`，并新增共享的 `设置` 页签。
- 研究中心2：提供独立的 `AI分析报告`、`股票推荐记录`、`股票收益率` 和共享的 `设置` 页签。
- 设置：不再单独占用侧边栏入口；两个研究中心看到的是同一套通用、数据接口、AI 分析和配置管理设置。
