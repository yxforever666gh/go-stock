...
研究中心2在交易日09:50启动，正常使用09:45—09:50的五个已闭合交易分钟；失败恢复可在 `[09:50,11:50)` 内启动，使用实际启动点之前最近5个已闭合交易分钟。每天最多生成一份有效报告，失败不占次数；成功或空推荐报告生成后不再补位分析。所有有效评分按分数降序参与执行，不设50分或主选/备选门槛，最多成交三只。每笔按当前现金与剩余名额弹性均分，一手成本可以超过均分目标，但含费总成本不得超过剩余现金。初始资金仍为12,000元，已有持仓市值不能当作现金使用。涨跌停距离、行情时效、交易窗口和次日卖出规则保持不变。策略版本为 `research2-trailing5-v10`，证据版本为 `research2-trailing5-v7`。旧版待买状态仅作兼容处理，全部旧待买结束后可移除该状态兼容；历史主备字段待对应旧记录全部归档后另行迁移删除。
