...
- 市场总结推荐增加候选池诊断、数量控制、报告合并和二轮补位，模型可生成更多候选，最终由后端硬规则统一拦截。
...
- 新增策略阶段 `V1.4.1`，市场总结新推荐默认写入 `summary_version = 1.4.1`，`current` 策略阶段同步指向 1.4.1。
- 市场总结推荐补齐二轮 AI 补位闭环：第一轮保存后若生产候选不足 2 只，会从剩余 verified candidates 中最多触发一次补位推荐表格生成。
- 二轮补位继续走原有解析、保存前交易计划硬校验、激活门控、同日去重和生产候选上限，不为了凑数放松风控。
- 新增市场总结运行诊断落库，记录候选池、AI 输出、保存结果、生产/仅分析数量、拦截原因 Top 5 和空转状态。
- `收益率统计` 页新增“市场总结诊断”面板，支持随策略阶段筛选查看最新诊断、拦截原因和最近连续空转次数。
- 候选池评分增强加入板块强度、近期失败记录和数据完整性，并输出 `scoreBreakdown` 供日志和诊断复盘。

### 1.4.0

- 新增策略阶段 `V1.4.0`，市场总结新推荐默认写入 `summary_version = 1.4.0`，不新增生产池、观察池、仅分析池三分法。
- 市场总结推荐前置候选池：复用指标选股能力，固定运行强势突破、量能确认、均线多头、回踩支撑和板块领涨模板，先形成 50 到 150 只左右的临时候选池。
- 候选池轻量评分后再交给 AI：按涨幅、量比、换手、成交额和价格区间排序，并把指标候选补入 AI discovery 结果，减少“AI 先乱选、硬规则再全拦”的空转。
- AI 最终推荐队列扩展到最多 12 只，并要求按优先级输出顺延补位交易计划；生产候选仍最多 4 只，硬规则不过仍不入库。
- 运行日志补充指标候选池数量、合并后候选数量和验证候选数量，方便复盘空转原因。

### 1.3.6

- 新增策略阶段 `V1.3.6`，市场总结新推荐默认写入 `summary_version = 1.3.6`，前端收益统计、收益列表和推荐记录页可切换查看。
- AI 推荐源头增加质量门槛：证据类别、高信任证据、价格锚点、技术/资金确认、负面信号和当前窗口新证据都进入候选过滤。
- 保存前增加交易计划硬校验：按最差可成交价检查盈亏比、止损空间和关键价位偏离，低质量计划自动降级为仅分析并保留明确原因。
- V1.3.6 激活门控改为“硬风控 + 强弱二次确认”：低于硬底线直接跳过，灰区需要 VWAP 强弱确认，单次止损不再立刻触发冷却。
- 历史市场总结修复按原记录版本解析旧报告，避免新阶段门槛误伤旧记录的止盈止损恢复。
...
- 新增独立策略阶段 `V1.3.2`，新推荐写入 `summary_version = v1.3.2`，历史 V1.3.1 数据保持隔离。
- V1.3.2 启用强弱过滤、盈亏比准入、重复止损冷却三件套硬规则，弱信号会跳过激活并记录原因。
- 收益率相关页面默认查看 `All`，可切换 `V1.3.2` 与 `V1.3.1` 对比不同策略阶段。
- `收益率统计` 页新增 V1.3.2 硬规则拦截诊断，区分强弱、盈亏比和冷却拦截。
