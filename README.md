...
schema 3 创建分析运行、推荐、生命周期消息、AI 决策事件、模拟账户、交易和持仓表；schema 4 增加逐模型启用状态；schema 5 增加脱敏的模型调用诊断记录；schema 7 增加生命周期证据快照；schema 8 将旧待激活记录迁移到一次性直接买入流程；schema 9 在永久归档后删除 1.5 旧策略表；schema 10 增加分期入金、资金预留、单位净值与策略评估快照；schema 11 增加分钟线接口顺序和持仓复查时间配置；schema 12 将研究中心1账户追溯重置为固定 50 万元；schema 13 新增研究中心2的独立 12,000 元账户、报告、推荐、成交、指标和自动开关；schema 14 增加研究中心2专属 SMTP 配置和持久化邮件投递队列。分钟库保持 schema 2。
