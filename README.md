...
- 当前 `1.2.6` 公开快照已主动移除个人赞赏码、联系方式、构建产物、运行数据库和本地私有接入说明，避免把不适合公开仓库的内容继续带到云端。
...
- 股票收益率页新增“查看全库收益走势”弹窗入口，用小窗展示全库按交易日的累计走势和单日变化，不再挤占主页面表格空间。
- 单日收益率口径改为按“当日参与盈亏计算的持仓成本”计算，不再被历史全量成本池摊薄；累计走势继续保留从头到尾的累计收益定义。
- 全库收益弹窗最后一个累计点已与页面顶部“总净收益率（仅统计可执行推荐）”对齐，避免同一时点出现两套 strict 结果。
- Web 模式补齐 `/build/appicon.png` 与 `/favicon.ico` 静态路由，弹窗和新页面会话下不再出现图标 404 干扰。
- `AI分析报告` 页继续显示 `Provider / 模型` 组合；从 `1.2.6` 起，新生成的市场资讯 AI 报告会稳定写入 provider 名称，不再只显示模型。
